document.addEventListener("DOMContentLoaded", () => {
  ...
  generateBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      alert(`Gagal membuat materi belajar: ${error.message}`);
      emptyState.classList.remove("hidden");
    } finally {
      btnText.textContent = "Buat Materi Belajar";
  ...
  function setupFlashcards(cardsArray) {
    ...
    if (!Array.isArray(cardsArray) || cardsArray.length === 0) {
      alert("Tidak ada flashcard yang dapat dibuat.");
      return;
    }
  ...
  function setupQuiz(questionsArray) {
    if (!Array.isArray(questionsArray) || questionsArray.length === 0) {
      alert("Tidak ada pertanyaan kuis yang dapat dibuat.");
      return;
    }
    ...
    function renderQuestion() {
      ...
      document.getElementById("quizProgress").textContent =
        `Pertanyaan ${currentQIndex + 1} dari ${questionsArray.length}`;
      ...
      questionsArray.forEach((q, i) => {
        ...
        feedbackHTML += `
                    <div class="feedback-item ${isCorrect ? "correct" : "incorrect"}">
                        <h4>P${i + 1}: ${q.question}</h4>
                        <p>Jawaban Anda: ${userAnswers[i]}</p>
                        <p class="correct-ans">Kunci Jawaban: ${q.correctAnswer}</p>
